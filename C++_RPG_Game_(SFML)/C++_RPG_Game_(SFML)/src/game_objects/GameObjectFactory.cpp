...
namespace RPG {
    namespace Factory {
        ...
        std::unique_ptr<GameObject> createPlayer(AssetManager& am, WorldMap* worldmap, sf::Vector2f pos) {
            ...
            player->setZIndex(static_cast<uint8_t>(LayerZIndex::ZLayer::Main));
        ...
        std::unique_ptr<GameObject> createNpc(AssetManager& am, sf::Vector2f pos, FactionID factionId, const std::string& textureId, int dialogId, bool isLeader) {
            ...
            npc->setPosition(pos);
            npc->setZIndex(static_cast<uint8_t>(LayerZIndex::ZLayer::Main));
        ...
        std::unique_ptr<GameObject> createBox(AssetManager& am, sf::Vector2f pos) {
            ...
            box->setPosition(pos);
            box->setZIndex(static_cast<uint8_t>(LayerZIndex::ZLayer::Main));
        ...
        std::unique_ptr<GameObject> createDynamicObject(AssetManager& am, const std::string& assetName, sf::Vector2f pos) {
            ...
            if (!tex) {
                std::cerr << "[Factory] FATAL: Missing texture '" << def.textureKey
                          << "' for object '" << assetName << "'!\n";
            }
            else {
