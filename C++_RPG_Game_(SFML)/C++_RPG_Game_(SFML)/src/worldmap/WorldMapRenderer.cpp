    ...
    void WorldMapRenderer::draw(sf::RenderWindow& window, const WorldMap& worldMap, const AssetManager& assetManager, const SpritesheetManager& sm, float percentView) {
        ...
        for (int16_t y = startY; y < endY; ++y) {
            for (int16_t x = startX; x < endX; ++x) {
                ...
                float dy = (tilePos.y + (drawWidth / 2.0f)) - center.y;
                float distance = std::sqrt(dx * dx + dy * dy);

                uint8_t alpha = 255;
                if (distance < maxVisibleDist) {
                    // inside of visible view
                    alpha = 255;
                }
                else if (distance < maxVisibleDist + fadeRange) {
                    // inside Fade zone (right after visible view)
                    float fadeFactor = (distance - maxVisibleDist) / fadeRange;
                    alpha = static_cast<uint8_t>(255.0f * (1.0f - fadeFactor));
                }
                else {
                ...
                // 5. Apply Texture & Color with alpha
